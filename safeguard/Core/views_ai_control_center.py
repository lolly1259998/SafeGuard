...
class ControlCenterPerformanceAI:
    ...
    def analyze_center_performance(control_center):
        ...
        """
        try:
            # Métriques de base (sans dépendre des caméras)
            performance_data = {
    ...
    def calculate_uptime(control_center):
        """Calcule le taux de disponibilité du centre"""
        ...
        # ✅ CORRECTION : Utiliser is_active au lieu de status
        ...
        return {
            ...
            'status_impact': 'active' if control_center.is_active else 'inactive'  # ✅ CORRECTION
        }
    
    @staticmethod
    def analyze_resources(control_center):
        """Analyse l'optimisation des ressources du centre"""
        resource_score = 75
    ...
    def assess_security_level(control_center):
        """Évalue le niveau de sécurité du centre"""
        security_factors = []
        security_score = 70
    ...
    def predict_maintenance(control_center):
        """Prédit les besoins en maintenance"""
        creation_date = control_center.created_at
    ...
    def analyze_energy_usage(control_center):
        """Analyse l'efficacité énergétique (simulée)"""
        base_efficiency = 65
...
def ai_performance_analysis(request, control_center_id):
    """Endpoint pour l'analyse IA des performances"""
    try:
...
def ai_recommendations(request, control_center_id):
    """Endpoint pour les recommandations IA"""
    try:
        from .models import ControlCenter
        control_center = ControlCenter.objects.get(id=control_center_id)
